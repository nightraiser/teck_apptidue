...
function createHomepageGoogleMap(json,callback){

    $.get("/external/_infobox.js", function() {
        gMap();
    });
    function gMap(){
        var mapCenter = new google.maps.LatLng(json.mapcenter[0].lat,json.mapcenter[0].lng);
        var mapOptions = {
            zoom: 14,
            center: mapCenter,
        ...
        var map = new google.maps.Map(mapElement, mapOptions);
          //Border detials---------------------------------------------
            ...
            var borderDetails = json.borderDetails;
            ...
            var locationBorder = new google.maps.Polygon({
            paths: borderDetails,
            strokeColor: '#ED1C24',
            strokeOpacity: 0.8,
            strokeWeight: 2,
    ...
    //Border details------------------------------------------------
        ...
        google.maps.event.addListener(map, 'idle', function() {
            ...
            {
                    $(".items-list .results1").html("<li><h1>0 Search Results found in the criteria</h1>Please try with a different search</li>");
            }
            else
            {
                var incVal = 0;
                ...
                if( map.getBounds().contains( new google.maps.LatLng( json.data[a].latitude, json.data[a].longitude ) ) ) {
                    ...
                    pushItemsToArray(json, a, category, visibleItemsArray,incVal);
                    incVal++;
                }
            });
                $('.items-list .results1').html( visibleItemsArray );
            }
            ...
            $singleItem.hover(
                function(){
                    console.log(newMarkers[ $(this).attr('attr')  ])
                    newMarkers[ $(this).attr('attr')  ].content.className = 'marker-active marker-loaded';
                },
                function() {
                    newMarkers[ $(this).attr('attr')  ].content.className = 'marker-loaded';
                }
            );
        });
...
// Push items to array and create <li> element in Results sidebar ------------------------------------------------------
 ...
 function pushItemsToArray(json, a, category, visibleItemsArray,incVal){
    var itemPrice;
    
    visibleItemsArray.push(
        '<li>' +
            '<div class="item" id="' + json.data[a].id + '" attr="'+incVal+'">' +
                '<a href="#" class="image">' +
                    '<div class="inner">' +
     ...
     '<div style="position: absolute;right: 4px;top: 4px;"><i class="fa fa-heart likeButton" data-score="1" style="font-size:24px;" data-id="'+json.data[a]['id']+'"></i></div>'+
                    '</div>' +
                '</a>' +
                '<div class="wrapper triggerPopup">' +
                    '<a href="#" id="' + json.data[a].id + '"><h3 title="'+ json.data[a].title+'">' + json.data[a].title.substr(0,20) + '..</h3></a>' +
                        '<div style="position:absolute; top:4px; right:4px;" class="res-rating">'+json.data[a].rating+'</div>' +
                    '<figure>' + json.data[a].location + '</figure>' +
                    drawPrice(json.data[a].price) +
                    '<div class="info">' +
                        '<div class="type">' +
                            '<p><strong>Dining Type: </strong>' + json.data[a].item_specific.diningStyle + '</p>' +
                            '<p><strong>Cusine: </strong>'+json.data[a].item_specific.cusine.toString().substr(0,28)+'..</p>' +
                        '</div>' +
                    '</div>' +
                '</div>' +
            '</div>' +
...
function multiChoice(sameLatitude, sameLongitude, json) {
    //if (clickedCluster.getMarkers().length > 1){
        ...
        var multipleItems = [];
        $.each(json.data, function(a) {
            if( (Math.abs(json.data[a].latitude - sameLatitude)<0.000002) && (Math.abs(json.data[a].longitude -sameLongitude)<0.000002) ) {
               var itemPrice;
                multipleItems.push(
                    '<li>' +
                        '<div class="item" id="' + json.data[a].id + '">' +
                            '<a href="#" class="image">' +
                                '<div class="inner">' +
                                    '<img src="' + json.data[a].gallery[0] + '" alt="">' +
                 '<div style="position: absolute;right: 4px;top: 4px;"></div>'+
                                '</div>' +
                            '</a>' +
                            '<div class="wrapper triggerPopup">' +
                                '<a href="/'+json.data[a].city_name+'/'+json.data[a].vanityUrl+'"><h3 title="'+ json.data[a].title+'">' + json.data[a].title.substr(0,20) + '..</h3></a>' +
                                    '<div style="position:absolute; top:4px; right:4px;" class="res-rating">'+json.data[a].rating+'</div>' +
                                '<figure>' + json.data[a].location + '</figure>' +
                                '<div class="info">' +
                                    '<div class="type">' +
        ...
        $('body').append('<div class="modal-window multichoice fade_in"><h1>Hello</h1></div>');
        var multi = "";
        multi += '<div class="modal-wrapper">';
        multi += '<h3>Select Property</h3>';
        multi += '<h2>Multiple Restaurants in one location</h2>';
        multi += '<div class="modal-body">';
        multi += '<ul class="items list-unstyled">'+multipleItems+'</ul>';
        multi += '</div>';
        multi += '<div class="modal-close"><img src="/images/close.png"></div>';
        multi += '</div>';
        multi += '<div class="modal-background fade_in"></div>';
        $('.multichoice').html(multi);
       /*
